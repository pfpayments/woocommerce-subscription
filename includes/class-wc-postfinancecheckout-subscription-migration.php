...
class WC_PostFinanceCheckout_Subscription_Migration {
	...
	public static function init(){
		...
		add_action('in_plugin_update_message-woo-postfinancecheckout-subscription/woo-postfinancecheckout-subscription.php', array(
			__CLASS__,
	...
	private static function check_requirements() {
		...
		if (!is_plugin_active('woo-postfinancecheckout/woocommerce-postfinancecheckout.php')){
		    $errors[] = sprintf(__("Woocommerce PostFinance Checkout %s+ has to be active.", "woo-postfinancecheckout-subscription"), WC_POSTFINANCECHECKOUT_SUBSCRIPTION_REQUIRED_POSTFINANCECHECKOUT_VERSION);
		}
		else{
			...
			if (version_compare ($base_module_data['Version'] , WC_POSTFINANCECHECKOUT_SUBSCRIPTION_REQUIRED_POSTFINANCECHECKOUT_VERSION, '<')){
			    $errors[] = sprintf(__("Woocommerce PostFinance Checkout %s+ is required. (You're running version %s)", "woo-postfinancecheckout-subscription"), WC_POSTFINANCECHECKOUT_SUBSCRIPTION_REQUIRED_POSTFINANCECHECKOUT_VERSION, $base_module_data['Version']);
			}
		}
		
		if (!is_plugin_active('woocommerce-subscriptions/woocommerce-subscriptions.php')){
		    $errors[] = sprintf(__("Woocommerce Subscriptions %s+ has to be active.", "woo-postfinancecheckout-subscription"), WC_POSTFINANCECHECKOUT_REQUIRED_WC_SUBSCRIPTION_VERSION);
		}
		else{
		    ...
		    if (version_compare ($woocommerce_subscriptions_data['Version'] , WC_POSTFINANCECHECKOUT_REQUIRED_WC_SUBSCRIPTION_VERSION, '<')){
		        $errors[] = sprintf(__("Woocommerce Subscriptions %s+ is required. (You're running version %s)", "woo-postfinancecheckout-subscription"), WC_POSTFINANCECHECKOUT_REQUIRED_WC_SUBSCRIPTION_VERSION, $woocommerce_subscriptions_data['Version']);
		    }
		}
		
		if(!empty($errors)){
			$title = __('Could not activate plugin WooCommerce PostFinance Checkout Subscription', 'woo-postfinancecheckout-subscription');
	...
	public static function in_plugin_update_message($args){
		$transient_name = 'postfinancecheckout_subscription_upgrade_notice_' . $args['Version'];
		
		if (false === ($upgrade_notice = get_transient($transient_name))) {
			$response = wp_safe_remote_get('https://plugins.svn.wordpress.org/woo-postfinancecheckout-subscription/trunk/readme.txt');
